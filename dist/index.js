...
async function CordovaRes(options = {}) {
    ...
    const { directory, resourcesDirectory, logstream, errstream, platforms, projectConfig, skipConfig, copy, operations: { fit = 'cover', position = 'center', transform = (image, pipeline) => pipeline, }, assetSuffix, } = { ...defaultOptions, ...options };
    ...
    for (const platform of platform_1.PLATFORMS) {
        ...
        if (platformOptions) {
            ...
            if (copy && nativeProject) {
                ...
                await native_1.copyToNativeProject(platform, nativeProject, resourcesDirectory, shouldCopyIcons, shouldCopySplash, assetSuffix, logstream, errstream);
            }
        }
    }
    if (config) {
